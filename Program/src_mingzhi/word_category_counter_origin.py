...
#Usage: python word_category_counter.py word_category_counter_origin.py text.txt
...
class Dictionary():
    ...
    '''Mingzhi edited this line because there is a bug'''
    # origin
    # _dictionary_line_re = re.compile(r'^(\w+)(\*?)\s*(.*)$')
    _dictionary_line_re = re.compile(r'^(\w+[\']?\w*]?)(\*?)\s*(.*)$')
    ...
    def load_dictionary_file(self, filename, internal_category_list=None):
        ...
        for line in open(filename):
            ...
            # if 'i'in word:
            #     print line
            #     print Dictionary._dictionary_line_re.match(line).groups()
            #     print word
            #     print self._lookup[word]
        ...
        if len(word) == 0:
            ...
            if word not in self._ignored:
                ...
                if word in self._lookup:
                    ...
                    # '''delete this line if you don't need to print out the category for each word'''
                    # print "=====type for this word===="
                    # print word
                    # print self._lookup[word]
...
_dictionary_filename = '/Users/mingzhiyu/Documents/LIWC_origini/Dictionaries/LIWC2007_English080730.dic'
...
def score_file(filename, raw_counts=False, scores=None, unique_words=None):
    ...
    # print "------Mingzhi debugs, this line should be remove at debugging------"
    ...
    return score_text(text, raw_counts=raw_counts, scores=scores, unique_words=unique_words)
...
def main():
    ...
    if not 2 <= len(sys.argv) <= 3:
        ...
        scores = score_file(filename)

        # for cat in scores:
        #     print cat + '\t' + str(scores[cat])

        pickle.dump(scores,open("scores.p","w"))
