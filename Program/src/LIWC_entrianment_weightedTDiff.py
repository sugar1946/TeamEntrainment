        ...
        for partID in ent.partIDs:
            ...
            if per_team:
                ...
                for word in words_per_team:
                    ...
                    print "print out this word"
                    print convergence[teamID][word + '@' + partID]
                    print teamID, word, partID

                    convergence_sum[teamID][method_name + '@' + partID] += convergence[teamID][word + '@' + partID]

                    #print convergence_sum[teamID][method_name + '@' + partID]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--score', '-s', action='store', dest='score',
                        help='The score file')
    parser.add_argument('--method', '-m', action='store',dest='method', help='the method name that will be displayed')
    ...
    if args.p:
        ...
        proximity_diff_df.to_csv(OUTPUT_PATH+'proximity_'+args.method+'_diff.csv')
        proximity_value_df.to_csv(OUTPUT_PATH+'proximity_'+args.method+'_value.csv')

    elif args.c:
        convergence_diff_df, convergence_value_df = convergence_sum_words(featureDF, words_per_team, args.method,\
                                                                          per_team=False)
        convergence_diff_df.to_csv(OUTPUT_PATH+'convergence_'+args.method+'_diff.csv')
        convergence_value_df.to_csv(OUTPUT_PATH+'convergence_'+args.method+'_value.csv')
