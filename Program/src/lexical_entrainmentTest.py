...
def proximity_per_word(featureDF):
    diffDic = ent.get_diff_subgroup(featureDF)
    proximity = ent.proximity_subgroup(diffDic)
...
#             print proximity_sum[teamID]
            ...
            for partID in ent.partIDs:
                ...
                if per_team:
                    for word in words_per_team[teamID]:
                        proximity_sum[teamID][method_name+'@'+partID+'@partnerDiff'] += proximity[teamID][word+'@'+partID+'@partnerDiff']
                        proximity_sum[teamID][method_name+'@'+partID+'@otherDiff'] += proximity[teamID][word+'@'+partID+'@otherDiff']
                else:
                    for word in words_per_team:
                        proximity_sum[teamID][method_name+'@'+partID+'@partnerDiff'] += proximity[teamID][word+'@'+partID+'@partnerDiff']
                        proximity_sum[teamID][method_name+'@'+partID+'@otherDiff'] += proximity[teamID][word+'@'+partID+'@otherDiff']
...
def proximity_sum_words(featureDF,words_per_team,method_name , intervals=False , per_team = True):
    ...
    for teamID in proximity:
        ...
        if not intervals:
            ...
            proximity_sum_value[teamID][method_name] = proximity_sum[teamID][method_name+'@otherDiff'] - proximity_sum[teamID][method_name+'@partnerDiff']
            print (proximity_sum_value[teamID][method_name])
        else:
            for partID in ent.partIDs:
                proximity_sum[teamID][method_name+'@'+partID+'@partnerDiff']=0.0
...
def convergence_sum_words(featureDF,words_per_team,method_name,per_team):
    ...
    for teamID in convergence:
        ...
        if per_team:
            ...
            wl = words_per_team
        for word in wl:
            if convergence[teamID][word+'@'+'interval1'] == 0.0 or convergence[teamID][word+'@'+'interval2'] == 0.0:
                convergence_sum[teamID][method_name+'@'+'interval1'] -= convergence[teamID][word+'@'+'interval1']
                convergence_sum[teamID][method_name+'@'+'interval2'] -= convergence[teamID][word+'@'+'interval2']   
                num -=1
            ...
            for partID2 in ent.partIDs:
                if int(partID[-1]) < int(partID2[-1]):
                    for ff in convergence_sum[teamID]:
                        ...
                        convergence_sum_value[teamID][f+'@'+partID+'_'+partID2] = (convergence_sum[teamID][f+'@'+partID] - convergence_sum[teamID][f+'@'+partID2]) 
...
featureDF_proximity = read_aggregate_frequency_file('/Volumes/litman/Transcriptions/Python/Lexical Entrainment/output/LIWC_Top25Words_corpus_Highest_funct.csv')
words_per_team = read_words('/Volumes/Litman/Transcriptions/Python/Lexical Entrainment/output/LIWC_Top25Words_corpus_Highest_funct', pergame=False)
# proximity_diff_df   , proximity_value_df = proximity_sum_words(featureDF_proximity, words_per_team, 'LIWC_Top25Words_corpus_Highest_funct',intervals= False , per_team = False)
# proximity_diff_df.to_csv('proximityDiff_LIWC_Top25Words_corpus_Highest_funct.csv')
# proximity_value_df.to_csv('proximity_LIWC_Top25Words_corpus_Highest_funct.csv')
