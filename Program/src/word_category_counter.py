...
#Usage: python word_category_counter.py word_category_counter.py text.txt
				...
				if word in self._lookup:
					...
					for entry_type in self._lookup[word]:
						...
						if entry_type == Dictionary._TYPE_BASIC:
							...
							# Mingzhi========
							for category in self._lookup[word][entry_type]:

								if category not in self.category_word:

									self.category_word[category] = []

								self.category_word[category].append(word)
							# Mingzhi========
					...
					for char in word:
						...
						if char in current_node:
							...
							if isinstance(current_node, set):
								...
								for category in self._lookup[word][Dictionary._TYPE_BASIC]:
...
# EDIT THIS LINE to the CORRECT PATH to LIWC2007 .dic FILE
#_dictionary_filename = os.path.abspath(os.path.join(os.path.dirname(__file__), 'data')) + '/LIWC2007.dic'
_dictionary_filename = '/Users/mingzhiyu/Documents/LIWC/Dictionaries/LIWC2007_English080730.dic'

_dictionary = Dictionary(_dictionary_filename, False, '2007')
...
def score_text(text, raw_counts=False, scores=None, unique_words=None):
	...
	for i in xrange(len(all_tokens)):
	...
	print _dictionary.category_word
	return (scores,_dictionary.category_word)


def score_file(filename, raw_counts=False, scores=None, unique_words=None):

	#=================Mingzhi added text standardized=============
	...
	return score_text(ret, raw_counts=raw_counts, scores=scores, unique_words=unique_words)
...
print _dictionary
print _dictionary_filename

#=====Mingzhi =====
def sort_by_frequency(list):
	# list of words that under certain catogory
	counter = Counter(list)
	sorted_by_frequence = counter.most_common(WORD_NUM) #(words,frenquency)
	#ret = [x[0] for x in sorted_by_frequence]

	return sorted_by_frequence

def writefile(filename,word_list,out_file):

	print filename
	p = re.compile('Team[0-9]*')
	...
	out_file.write(teamID+"\n")

	#tup (u'word',0.09986)
	for tup in word_list:		
		out_file.write(str(tup[0])+"\n")
...
def main():
	...
	if not 2 <= len(sys.argv) <= 3:
		...
		scores,category_word = score_file(filename) 

		#Mingzhi Modified
		Maxi = 0
		topic = ''
		for cat in scores:
			print cat + '\t' + str(scores[cat])

			if Maxi < scores[cat] and cat[0].islower():
		...
		topic_words =  sort_by_frequency(category_word[topic])

		output_file = open('LIWC_Top25Words_corpus_12_4(2)','a')
		writefile(filename,topic_words,output_file)
		output_file.close()
