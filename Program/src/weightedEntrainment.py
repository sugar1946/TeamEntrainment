...
def team_total_member(featureDf):
    num_dic = {}
    for index, row in featureDf.iterrows():
        print "====Group Num===="
        speaker_group = get_group_ID(index)
        ...
        if speaker_group in num_dic:
            num_dic[speaker_group] += 1

        else:
            num_dic[speaker_group] = 1

    print "====Group: number of members===="
    print num_dic
                    ...
                    if speaker_game == get_game_number(index2):
                        ...
                        for feature in row.index:
                            ...
                            if not feature in ['#total'+ext]:
                                ...
                                if speaker_group == '9999':
                                    print "-------Testing---------"
                                    print index,index2
                                    print feature
                                    print row[feature],row2[feature]
                                    print diffDic[index][index2][feature]
    ...
    for speaker in diffDic.keys():
        ...
        for speaker2 in diffDic[speaker].keys():
            ...
            if not get_Individual_ID(speaker) == get_Individual_ID(speaker2):
                ...
                if teamID == '9998':
                    print diffDic[speaker][speaker2].keys()
        ...
        # print "####avgDic_game1[teamID].keys()####"
        # print avgDic_game1[teamID].keys()

        for f in avgDic_game1[teamID].keys():  

            convergance_game1[teamID][f] =  avgDic_game1[teamID][f]/((numc1[teamID]+1) * numc1[teamID])
            if teamID == '9998':

                print "======new test numc1[teamID]======="
                #print len(avgDic_game1[teamID].keys())
                #print teamID
                print f
                print avgDic_game1[teamID][f]
                print convergance_game1[teamID][f]
                print numc1[teamID]
